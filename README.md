...
- **타법인 출자현황의 수량 단위 오기 주의.** 세아메카닉스 2025 사업보고서는 표 머리글이
  `(단위 : 백만원, 천주, %)`인데 두산에너빌리티·현대자동차 행만 수량을 *주* 단위로 적었다
  (삼성전자 행은 천주로 정상). OpenDART API는 머리글 단위대로 ×1,000 해서 돌려주므로
  그대로 믿으면 보유수량이 1,000배로 부풀려진다. 세아메카닉스는 이 경로로 두산에너빌리티
  `sharesHeld`가 2,550,000주(→ 지분가치가 별도 자산총계 1,594억을 초과)로 잘못 들어가
  있었고, 2,550주로 바로잡았다(2026-07-29). 검증 근거는 기말 장부가액 192백만원 ÷ 2,550주
  = 75,294원 ≈ 2025-12-30 종가 75,300원, 그리고 지분율 기재값 0.00%. 예외는
  `fundamentals_overrides.json`의 `skipQtyCheck`로 고정했다.
- 세아메카닉스는 지주회사가 아니라 단순투자 목적으로 두산에너빌리티 2,550주(1.6억원)를
  들고 있을 뿐이라, 정정 후 비율이 0.16%까지 떨어졌다. 지주사 대시보드의 취지에는 맞지
  않는 페어이므로 존치 여부를 재검토할 것.
