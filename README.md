...
| `css/` | 프런트 스타일 |
| `data/summary.json` | 생성 산출물 — 메타+현재가+최근 3년 백분위용 날짜·비율. 프런트 첫 화면의 데이터 소스 |
| `data/fundamentals.json` | 생성 산출물 — 종목별 별도 자본총계·지분 장부가액·잔존자본 (원 단위) + 자회사별 출자현황 상세(`detail`: 기말 지분율·기중 취득/처분·피출자법인 최근사업연도 순이익). 실질가치 지표와 "보유 지분 상세" 표용 |
...
- **실질가치** (현재 상태 전용 지표, 추이 없음):
  ...
  - 잔존자본은 **음수가 될 수 있다** (지분 장부가액 > 별도 자본총계 = 차입으로 지분 취득).
    그대로 음수 비율로 표시한다.
- **백분위 칩**: 카드에 표시된 최신 비율이 최근 1년/3년 분포에서 차지하는 백분위(%).
  `data/summary.json`의 `percentileHistory`(최근 3년 날짜·비율)로 전체 히스토리 로드 전에도
  계산하며, 실시간 시세가 들어오면 같은 날짜는 교체하고 새 날짜는 추가한다. 저장된
  `pctile1y`/`pctile3y`는 일별 종가 참고값이며 UI에서는 사용하지 않는다.
  3년 창은 730일 이전 주간 다운샘플 구간을 포함하므로 근사치다. 표본 30개 미만이면 생략.
  상세의 **백분위 (그래프 구간)**는 선택 기간과 확대 범위를 기준으로 하므로 카드와
  다를 수 있다. 두 화면 모두 해당 구간 표본 중 현재 비율 이하인 비중을 정수로 반올림한다.
